...
Endpoint Security      Network Extension   [both data-path adapters built;
                                             network SYSX buildable unsigned]
...
protected out-of-band publisher, read-only provider startup, bounded containing-app preference
controller, unsigned containing-app/System Extension product graph, and activation/inspection/
replacement/deactivation coordinator are built. Signing and entitled-device activation are not,
so this is still not installed enforcement. A four-plane health evaluator requires activation,
exact preferences, authenticated provider readiness, and generation-matched flow proof before it
can report fully enforced. See ADRs 0035–0036 and 0043–0047 and the
...
Process-tree termination is an explicit opt-in response. Immediately before `SIGTERM` and again
before any `SIGKILL`, VIGIL compares the live PID's kernel-reported start time and executable with
the values captured at spawn, deepest child first. Missing or changed identity refuses the signal
and is reported rather than risking an unrelated process. This is stronger than PID identity but
still weaker than Endpoint Security audit-token identity; processes absent from the graph remain
outside its reach. See ADR 0041.
...
database can rewrite the whole chain. Manual Ed25519 checkpoints can now anchor a sequence/head
pair outside the chain and expose that rewrite to a verifier holding the public key. Checkpoints
are not automatic, and a key stored beside the database does not protect against an attacker who
can read both. See ADR 0040.
...
This is not yet an installed System Extension. Xcode is installed, but Apple entitlement approval,
bundle targets, containing-app activation, XPC Mach-service/daemon packaging, protected key and
code-requirement provisioning, and privileged-device testing remain Phase 3 work. Deadline misses
and queue drops are security health signals, not ordinary telemetry noise.

The network filter data provider accepts only an already authenticated compact `vigil-network`
snapshot. Rust signing, Swift verification, protected shared-container publication, read-only
provider startup, and containing-app preference management are built because the public SDK makes
the filter control provider unavailable on macOS. The design does not assume unrestricted
network, database, IPC, or shared-container writes from the data provider. The containing app now
owns the public activation/inspection/replacement/deactivation request lifecycle, with downgrade
refusal and truthful approval/reboot/failure states. Signed activation and entitled-device
notification/health remain unverified.
